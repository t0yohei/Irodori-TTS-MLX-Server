...
| Speech endpoint route | `POST /v1/audio/speech` returns a complete audio response and can stream audio or SSE depending on `stream_format`. | `POST /v1/audio/speech` returns a complete audio response or SSE audio deltas. | Partial | Streaming now uses the OpenAI-compatible speech route instead of a server-specific route, but only SSE streaming is supported for now. |
...
| Streaming response / SSE | Supports OpenAI speech streaming controls such as `stream_format`. | Supports `stream_format="sse"` or SSE `Accept` requests for `speech.audio.delta` / `speech.audio.done` events on `/v1/audio/speech`. `stream_format="audio"` is not supported. | Partial | SSE mode emits one base64 audio delta per synthesized text chunk, so it is chunk-level rather than frame-level runtime streaming. Audio byte streaming is left out until it can match the expected semantics without overstating compatibility. |
...
| Long-text chunking | Automatically chunks long text, with request/default controls such as `chunking_enabled` and `chunk_min_chars`; skips chunking when explicit seconds are set. | Chunks by punctuation after `chunk_min_chars` non-space characters, then falls back to hard character slices. The public request controls are `irodori.chunking_enabled`, `irodori.punctuation_chunking_enabled`, and `irodori.chunk_min_chars`; `punctuation_chunking_enabled` is accepted as a compatibility flag for the default punctuation planner. Explicit `irodori.seconds` is distributed across chunks by character count. | Partial | Use the public request controls for chunking behavior. |
...
`voice={"id":"<id>"}` to `irodori.ref_wav`, and explicit
`irodori.ref_wav` only when the path resolves to an existing managed file.
...
- Implemented: managed common audio upload extensions, `voice={"id":"..."}`
  normalization, and managed-root validation for explicit `irodori.ref_wav`.
...
non-streaming speech download, speech streaming, and response format conversion,
plus upstream-style request fixtures for supported options and stable
OpenAI-style error codes for unsupported behavior.
