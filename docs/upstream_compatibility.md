...
| Reference voice support | Resolves voices from files, `voices.json`, HTTP uploads, or a voice object. | Per-request `irodori.reference_wav` can be passed to the MLX runtime when `irodori.no_reference=false`. | Partial | There is no persistent voice registry, alias file, upload API, or voice object normalization yet. |
...
| Voice management endpoints | Provides `GET`, `POST`, `GET by id`, `PUT`, and `DELETE` routes under `/v1/audio/voices`. | No `/v1/audio/voices` routes. | Unsupported | This is the biggest upstream API gap for voice-library clients and should be handled by a dedicated voice/reference management issue. |
