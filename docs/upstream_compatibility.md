...
| Speech endpoint route | `POST /v1/audio/speech` returns a complete audio response and can stream audio or SSE depending on `stream_format`. | `POST /v1/audio/speech` returns a complete audio response, audio byte streams, or SSE audio deltas depending on `stream_format`. | Implemented | Streaming now uses the OpenAI-compatible speech route instead of a server-specific route. |
...
| Streaming response / SSE | Supports OpenAI speech streaming controls such as `stream_format`. | Supports `stream_format="audio"` for audio byte streaming and `stream_format="sse"` or SSE `Accept` requests for `speech.audio.delta` / `speech.audio.done` events on `/v1/audio/speech`. | Implemented | Audio byte streaming preserves a single valid output file. SSE mode emits one base64 audio delta per synthesized text chunk, so it is chunk-level rather than frame-level runtime streaming. |
