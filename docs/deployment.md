...
| `IRODORI_MLX_CODEC_RUNTIME_MODE` | `mlx` | MLX DACVAE codec runtime mode. `mlx` uses the MLX DACVAE artifact for both encode and decode. `mlx-decode` and `mlx-decode-subprocess` remain available for focused runtime comparisons. |
